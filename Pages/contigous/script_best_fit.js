        ...
        for (let j = 0; j < memoryBlocks.length; j++) {
            if (memoryBlocks[j].size - memoryBlocks[j].allocatedSize >= processSize) {
                ...
                if (fragmentation < minFragmentation) {
                    ...
                    minFragmentationIndex = j;
                }
            }
        }
        if (minFragmentationIndex !== -1) {
            memoryBlocks[minFragmentationIndex].allocated = true;
            memoryBlocks[minFragmentationIndex].allocatedProcesses.push(i + 1);
            memoryBlocks[minFragmentationIndex].allocatedSize += processSize;
        } else {
